(function($) {
    ...
    -------------------------------------------*/
    if ($("#clock").length) {
        $('#clock').countdown('2024/07/14', function(event) {
    ...
    -------------------------------------------*/
    $.validator.addMethod("validInviteCode", function(value, element) {
        return this.optional(element) || value === "220198" || value === "291299";
    }, "Invite code must be either 220198 or 291299.");
    if ($("#rsvp-form").length) {
        $("#rsvp-form").validate({
            rules: {
                name: {
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: "POST",
                    url: "https://script.google.com/macros/s/AKfycbySCTdjF4iPt4FmQc2gPoT0YZRY3UvqAQvLHbHPDH_B5rv2H6bqOddm8hD0ll74FwIsLw/exec",
                    data: $(form).serialize(),
                    statusCode: {
                        302: function() {
                            // Handle the redirect
    ...
    function showQRCode(person) {
        let qrCodeSrc = '';
        if (person === 'groom') {
            qrCodeSrc = 'images/qr_nam.jpg';
        } else if (person === 'bride') {
            qrCodeSrc = 'images/qr_thuy.jpg';
        }
